...
public class ApplicationMaster {
    ...
    private static final String ExecShellStringPath = YarnClient.SCRIPT_PATH + ".sh";
    private static final String ExecBatScripStringtPath = YarnClient.SCRIPT_PATH
            + ".bat";
    ...
    public boolean init(String[] args) throws ParseException, IOException {
        ...
        opts.addOption("session", true, "Session ID");
        opts.addOption("zk_quorum", true, "zk quorum");
        opts.addOption("docker_image", false, "docker_image");
        ...
        session = cliParser.getOptionValue("session");
        zkQuorum = cliParser.getOptionValue("zk_quorum");
        ...
        // Creating the Timeline YarnRClient
    ...
    public void run() throws YarnException, IOException {
        ...
        LOG.info("Submitter user name = " + appSubmitterUserName);
        appSubmitterUgi =
                UserGroupInformation.createRemoteUser(appSubmitterUserName);
        LOG.info("Submitter user obj = " + appSubmitterUgi);
        appSubmitterUgi.addCredentials(credentials);
        LOG.info("Submitter credentials = " + credentials);

        AMRMClientAsync.CallbackHandler allocListener = new RMCallbackHandler();
        amRMClient = AMRMClientAsync.createAMRMClientAsync(1000, allocListener);
        amRMClient.init(conf);
        amRMClient.start();
        LOG.info("aRMClient setup...");
        containerListener = createNMCallbackHandler();
        nmClientAsync = new NMClientAsyncImpl(containerListener);
        nmClientAsync.init(conf);
        nmClientAsync.start();
        LOG.info("nmClient start");
        ...
        LOG.info("app master hostname = " + appMasterHostname + "/" + amRMClient.getClass());
        RegisterApplicationMasterResponse response = amRMClient
                .registerApplicationMaster(appMasterHostname, appMasterRpcPort,
                        appMasterTrackingUrl);
        LOG.info("AMRM response = " + response);
    ...
    private class LaunchContainerRunnable implements Runnable {
        ...
        public void run() {
            ...
            String jar = null;
            try {
                jar = URLDecoder.decode(path, "UTF-8");
            } catch (UnsupportedEncodingException e) {
                throw new RuntimeException("Unable to find current jar!", e);
            }
            vargs.add(Environment.JAVA_HOME.$() + "/bin/java -cp " + jar + " com.hortonworks.datascience.Server");
            // Set executable command
            vargs.add("-e " + session);
            vargs.add("-q " + zkQuorum );
            if(dockerImage != null && !dockerImage.isEmpty())
            {
               vargs.add("-d " + dockerImage);
            }

            // Add log redirect params
            //vargs.add(">& /dev/stdout");
